...
$ThreadSearchCon = new Bbs\Controller\ThreadSearch();
$threadMod = new Bbs\Model\Thread();
$commentMod = new Bbs\Model\Comment();
$threads =$ThreadSearchCon->run();
?>
<h1 class="page__ttl">スレッド名検索</h1>
<form action="" method="get" class="form-group form-search">
  <div class="form-group">
    <input type="text" name="keyword" value="<?= isset($ThreadSearchCon->getValues()->keyword) ? h($ThreadSearchCon->getValues()->keyword): ''; ?>" placeholder="スレッド検索">
    <p class="err"><?= h($ThreadSearchCon->getErrors('keyword')); ?></p>
  </div>
  <div class="form-group">
...
<?php if (($ThreadSearchCon->getErrors('keyword'))): ?>
<?php else : ?>
  ...
  <?php foreach($threads as $thread): ?>
    <li class="thread__item">
      ...
      <ul class="thread__body">
        <?php
          $comments = $commentMod->getComment($thread->id);
      ...
      <div class="operation">
        <a href="<?= SITE_URL; ?>/thread_disp.php?thread_id=<?= $thread->id; ?>">書き込み&すべて読む(<?= h($commentMod->getCommentCount($thread->id)); ?>)</a>
